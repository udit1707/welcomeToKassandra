...
export default function Meetings(props) {
{
    ...
    useEffect(()=>{
        ...
        try{
            ...
            console.log(token);
            if (token!==null){
                fetch('http://welcome-to-kassandra.azurewebsites.net/employee/myMeetings?date="2021-06-04"',{headers:{Authorization: 'Bearer ' + token}})
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView style={{flex:1}}>    
          {
            data.meetings.map((meetInfo,index)=>{
              return(
                <Card containerStyle={styles.card} id={meetInfo.meeting.id}>
                  <View style={styles.cardAlignment}>
                    <View style={styles.cardImageView}>
                      <Image source={require("../assets/manufacturing.jpg")} style={styles.cardImage}/>
                    </View>
                    <View style={styles.infoSection}>
                      <Card.Title>{meetInfo.meeting.subject}</Card.Title>
                      <Card.Divider style={styles.cardDivider}/>
                      <Text>Happiness Index : {meetInfo.meeting.dated}</Text>
                      <View style={styles.cardButtonView}>
                        <TouchableOpacity style={styles.cardButton} onPress={()=>{configure(meetInfo.meeting.id-1)}}>
                          <Text>MoreInfo</Text>
                            <Modal isVisible={modalVisible} style={styles.modal} backdropOpacity={0.35}>
                              <View style={styles.modalView}>
                                <Text style={styles.modalTitle}>{data.meetings[key].employer.org_name}</Text>
                                <Text style={styles.modalText}>{data.meetings[key].employer.type}</Text>
