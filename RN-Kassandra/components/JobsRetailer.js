...
export default function JobsRetailer(props) {
{
    const RetailJobs=[];
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView style={{flex:1}}>    
          {
            data.Retailer.map((retInfo,index)=>{
              return(
                <Card containerStyle={styles.card} id={retInfo.id}>
                  <View style={styles.cardAlignment}>
                    <View style={styles.cardImageView}>
                      <Image source={require("../assets/retai.jpg")} style={styles.cardImage}/>
                    </View>
                    <View style={styles.infoSection}>
                      <Card.Title>{retInfo.org_name}</Card.Title>
                      <Card.Divider style={styles.cardDivider}/>
                      <Text>Happiness Index : {retInfo.employee_happ_index}</Text>
                      <View style={styles.cardButtonView}>
                        <TouchableOpacity style={styles.cardButton} onPress={()=>{configure(retInfo.id-1)}}>
                          <Text>MoreInfo</Text>
                            <Modal isVisible={modalVisible} style={styles.modal} backdropOpacity={0.35}>
                              <View style={styles.modalView}>
                                <Text style={styles.modalTitle}>{data.Retailer[key].org_name}</Text>
                                <Text style={styles.modalText}>{data.Retailer[key].email}</Text>
                                <Text style={styles.modalText}>{data.Retailer[key].address}</Text>
