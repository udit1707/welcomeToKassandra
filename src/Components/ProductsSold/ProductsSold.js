...
const ProductsSold= props => {
    ...
    const fn2 = useCallback(async ()=>{
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (item.retailerStock > 0){
                data.push({
                    src:item.image,itemCategory:item.category,itemDate:date,itemTime:strTime,
                    itemStatus:"Processing",itemAddress:item.retailerStock,buyer:'retailers'
                })
            }
            if (item.amazonStock > 0){
                data.push({
                    src:item.image,itemCategory:item.category,itemDate:date,itemTime:strTime,
                    itemStatus:"Processing",itemAddress:item.amazonStock,buyer:'Amazon'
                })
            }
            if (item.ebayStock > 0){
                data.push({
                    src:item.image,itemCategory:item.category,itemDate:date,itemTime:strTime,
                    itemStatus:"Processing",itemAddress:item.ebayStock,buyer:'Ebay'
                })
            }
            if (item.flipkarStock > 0){
                data.push({
                    src:item.image,itemCategory:item.category,itemDate:date,itemTime:strTime,
                    itemStatus:"Processing",itemAddress:item.flipkarStock,buyer:'Flipkart'
                })
            }        
        };
        setNewData(data)                    
